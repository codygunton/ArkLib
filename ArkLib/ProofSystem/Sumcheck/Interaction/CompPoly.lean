...
/-! ### Local bridge lemmas for the current CompPoly pin

The following facts are the Sumcheck-facing slice of the upstream CompPoly API
that this module needs. Evaluation through `bind₁` is proved from the existing
`CMvPolynomial`/`MvPolynomial` equivalence. The degree-preservation and
iterated-sum facts are kept as explicit assumptions until the corresponding
CompPoly API lands upstream and ArkLib can depend on it directly.
-/

/-- The computable substitution `bind₁` agrees with Mathlib substitution after
transporting through `fromCMvPolynomial`. -/
private lemma fromCMvPolynomial_bind₁ {m : ℕ}
...
theorem partialEvalFirst_individualDegreeLE [Nontrivial R] {deg : ℕ} (a : R)
  ...
  have hSupport :
    ...
    apply Finset.sup_le
    intro mono hmono
    exact hDeg i.succ mono (by simpa using hmono)
  have hEval :
...
private lemma partialEvalLast_degreeOf_le [Nontrivial R] {deg : ℕ} (a : R)
...
private lemma sumOverLast_degreeOf_zero_le [Nontrivial R] {deg : ℕ} (D : Fin m → R)
...
private lemma sumAllButFirst_degreeOf_zero_le [Nontrivial R] {deg : ℕ} (D : Fin m → R) :
...
private theorem toUnivariate_toPoly (p : CMvPolynomial 1 R) :
  ...
  have h := MvPolynomial.map_eval₂Hom
    ...
    (φ := (CompPoly.CPolynomial.ringEquiv (R := R)).toRingHom)
    (p := fromCMvPolynomial p)
  have hcomp :
      ((CompPoly.CPolynomial.ringEquiv (R := R)).toRingHom).comp
        (cPolyRingHom (R := R)) = Polynomial.C := by
    ext r n
    rw [RingHom.comp_apply]
  ...
  have hvars :
      (fun i : Fin 1 =>
          (CompPoly.CPolynomial.ringEquiv (R := R)).toRingHom CPolynomial.X) =
        (fun _ : Fin 1 => Polynomial.X) := by
    funext i
    change CPolynomial.X.toPoly = Polynomial.X
    rw [CPolynomial.X_toPoly]
  rw [eval₂_equiv (p := p) (f := cPolyRingHom (R := R))
    (vals := fun _ : Fin 1 => CPolynomial.X)]
  change (CompPoly.CPolynomial.ringEquiv (R := R)).toRingHom
      (MvPolynomial.eval₂ (cPolyRingHom (R := R)) (fun _ : Fin 1 => CPolynomial.X)
